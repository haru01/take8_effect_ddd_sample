...
- [x] E2Eテスト完全実装 (`tests/stories/course-addition.e2e.test.ts`)
  - [x] 科目一括追加の成功シナリオ
  - [x] 単位数上限超過の失敗シナリオ  
  - [x] 重複科目追加の失敗シナリオ
...
- [x] 関数型バリデーションビルダーパターン実装
- [x] アプリケーションコマンド完全実装 (`AddCoursesToSessionCommand`)
- [x] カスタムアサーション拡張完了（科目追加用テストヘルパー）
- [x] 統合テストと動作確認完了

**アーキテクチャ成果**:
- **関数型パターン**: バリデーションビルダーで組み合せ可能なビジネスルール
- **イベント統合**: 科目追加と履修要求を単一イベントで処理
...
- [ ] E2Eテストファイル作成 (`tests/stories/session-submission.e2e.test.ts`)
  - [ ] 履修登録提出の成功シナリオ
  - [ ] 最小単位数不足の失敗シナリオ
  - [ ] 不正状態での提出失敗シナリオ
...
**実装方針**:
- **ストーリー2パターン流用**: バリデーションビルダー・イベント駆動アーキテクチャ
- **既存基盤活用**: EventStore, EventBus, Repositoryの再利用
- **関数型設計**: 組み合せ可能なビジネスルールバリデーション
