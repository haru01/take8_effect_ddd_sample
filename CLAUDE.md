...
4. **品質向上**: `refactor-committer` で必要に応じてリファクタリング
5. **品質検証**: `qa-committer` でテスト品質確保

#### 複雑な機能開発時
```bash
# 段階的なエージェント連携
/domain-expert "履修完了・成績付与機能の要件整理"
/design-task-committer "前回の要件に基づく技術設計"
/task-committer "設計に基づく段階的実装"
...
```bash
# 専門エージェントによる課題解決
/refactor-committer "イベントストアのパフォーマンス改善"
/qa-committer "改善後の品質・パフォーマンス検証"
```
